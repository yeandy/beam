...
# batching flag added
class HuggingFaceStripBatchingWrapper(MegatronBertForMaskedLM):
...
class PostProcessor(beam.DoFn):
  ...
  def process(self, element: Tuple[str, PredictionResult]) -> Iterable[str]:
    ...
    mask_token_index = torch.nonzero(
        inputs['input_ids'] == BERT_TOKENIZER.mask_token_id,
        as_tuple=False).squeeze(-1)
    predicted_token_top_k_ids = torch.topk(logits[mask_token_index],
                                           3).indices[0]
    decoded_words = [
        BERT_TOKENIZER.decode(predicted_token_top_k_ids[i:i + 1])
        for i in range(len(predicted_token_top_k_ids))
    ]
    yield text + ';' + str(decoded_words)
...
def run(argv=None, model_class=None, model_params=None, save_main_session=True):
  ...
  if not model_class:
    model_config = MegatronBertConfig(
        vocab_size=30522, is_decoder=False, return_dict=True)
